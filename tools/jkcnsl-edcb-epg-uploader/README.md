...
`追加` は EDCB のサービス一覧を開いて、送信対象チャンネルを選んで追加するボタンです。`ONID / TSID / SID` は EDCB 由来の識別子なので、一覧では読み取り専用です。

保存や自動起動登録の成功メッセージは、設定ウィンドウ右下のステータスに表示されます。WinForms のシステム配色追従ダークモードは `.NET 10` 以降を前提にしています。
