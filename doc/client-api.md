...
メモリ内キャッシュから、1放送日分のEPGデータを返します。このAPIを呼び出しても、外部の TVer、NHK、AT-X、放送大学APIへのアクセスは発生しません。
...
要求された放送日のデータがメモリに読み込まれていない場合、`loaded` は `false` になります。サーバーは取得できている場合、TVerデータを現在の放送日と翌放送日ぶん保持します。一方、NHKなど一部のデータは現在の放送日分だけ存在する場合があります。キャッシュ済み番組がないチャンネルは空の `programs` 配列を返すため、クライアント側では「未取得」と表示してください。
